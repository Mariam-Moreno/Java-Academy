...
public class SimpleStack<T> implements Iterable<T> {
    ...
    public void push(T item) {
        // Crear nuevo nodo que apunte al top actual y actualizar top
        top = new Node<>(item, top);
        size++; // actualizar size
    }

    public T pop() {
        if (isEmpty()) throw new NoSuchElementException("Stack vacio");
        // guardar dato del top, avanzar top, decrementar size
        T data = top.data;
        top = top.next;
        size--;
        return data;
    ...
    private class StackIterator implements Iterator<T> {
        ...
        public T next() {
            ...
            // guardar dato, avanzar current, retornar dato
            T data = current.data;
