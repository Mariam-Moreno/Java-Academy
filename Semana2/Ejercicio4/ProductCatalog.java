...
record Product(String name, double price, String category, boolean inStock) {
    ...
    String toDisplayString() {
        return String.format(Locale.US, "%-15s $%7.2f  %-12s [%s]",
...
class ProductPipeline {
    ...
    public void forEach(List<Product> products, Consumer<String> action) {
        // filtrar productos, aplicar transformacion, ejecutar accion
    ...
    public long count(List<Product> products) {
        // contar productos que pasan el filtro
        long total = 0;
        for (Product p : products) {
...
public class ProductCatalog {
    public static void main(String[] args) {
        // Supplier para crear datos de prueba
        ...
        // usar method reference System.out::println
        catalogo.stream()
            .map(Product::toDisplayString)
            .forEach(System.out::println);

        System.out.println("\n=== Pipeline: Electronica en stock, precio > $50 ===");
        // crear pipeline con 3 condiciones encadenadas
        new ProductPipeline()
            .where(Product::isAvailable)          // method reference #1
            .where(p -> p.category().equals("Electronica"))
            .where(p -> p.price() > 50)
            .forEach(catalogo, System.out::println); // method reference #2
        ...
        pipeline.forEach(catalogo, System.out::println); // method reference #3
